...
class iMap(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.export_btn = tk.Button(self, text="Export", command=self.export, width=20)
    ...
    def export(self):
        ...
        if (self.img_folder.get() and self.export_folder.get()):
            if (os.path.isdir(self.img_folder.get()) and os.path.isdir(self.export_folder.get())):
                ...
                for image in images:
                    obj = []
                    obj.append(image.get_date_created())
                    obj.append(image.get_original_name())
                    obj.append(image.get_lat())
                    obj.append(image.get_lon())
                    obj.append(image.get_address())
                    obj.append(image.get_new_name())
                    data.append(obj)
